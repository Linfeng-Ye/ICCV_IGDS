...
class Randompatch(object):
    def __init__(self, transform=nn.Identity()):
        self.transform = transform
    def __call__(self, sample):
        patches = sample.unfold(1, 128, 128).unfold(2, 128, 128)
        patches = patches.flatten(1,2)
        Rtimg = torch.zeros_like(sample)
        idx = [i for i in range(4)]
        random.shuffle(idx)
        patches = patches[:,idx,:,:]
        # patches = patches.reshape(3,2,2,128, 128)
        Rtimg[:, 0:128, 0:128] = self.transform(patches[:, 0, :,:])
        Rtimg[:, 0:128, 128:256] = self.transform(patches[:, 1, :,:])
        Rtimg[:, 128:256, 0:128] = self.transform(patches[:, 2, :,:])
        Rtimg[:, 128:256, 128:256] = self.transform(patches[:, 3, :,:])
        return Rtimg
...
def transform_imagenet(size=-1,
    ...
    if size > 0:
        ...
        resize_train = [transforms.RandomResizedCrop(128)]
        # resize_test = [transforms.Resize(256), transforms.CenterCrop(224)]
    
    # resize_test = [transforms.Resize(368), transforms.CenterCrop(322)]
    resize_test = [transforms.Resize(256), transforms.CenterCrop(224)]

    if args.net_type == 'convnet':
        resize_test = [transforms.Resize(256), transforms.CenterCrop(256)]
    ...
    if Patches:
        ...
        train_transform = transforms.Compose(cast +[Randompatch(transforms.Compose(aug))]+resize_train + normal_fn)
...
def load_resized_data(args):
    ...
    if args.dataset == 'cifar10':
        ...
        _, test_transform = transform_imagenet(size=args.size, args=args)
        train_dataset = ImageFolder(traindir,
                                    transform=transform,
                                    nclass=args.nclass,
                                    phase=args.phase,
