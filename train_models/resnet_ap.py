...
class ResNetAP(nn.Module):
    def __init__(self, dataset, depth, num_classes, width=1.0, norm_type='batch', size=-1, nch=3, Patch=False):
        ...
        if self.Patch:
            ...
            self.fc = nn.Conv2d(in_channels=512 * blocks[depth].expansion,
    ...
    def get_feature(self, x, idx_from=0, idx_to=-1):
        if idx_to == -1:
            idx_to = idx_from
