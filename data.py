...
class Randompatch(object):
    ...
    def __call__(self, sample):
        ...
        # print(patches.shape, flush=True)
        if idx ==0:
            return patches[:,0,0,:,:]
        if idx ==1:
            return patches[:,0,1,:,:]
...
def transform_imagenet(size=-1,
                       ...
                       rrc=True,
                       rrc_size=-1):
    print("========================================================================================Size {}".format(size), flush=True)
    ...
    # breakpoint()
    if Patches:
        train_transform = transforms.Compose(cast +[Randompatch(), transforms.Resize(256)]+resize_train +  aug + normal_fn)
