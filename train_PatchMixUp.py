...
from data_F import load_data, MEANS, STDS
from misc.utils import random_indices, rand_bbox, AverageMeter, accuracy, get_time, Plotter, rand_bbox_patch
...
def train(args, model, train_loader, val_loader, plotter=None, logger=None):
    ...
    for epoch in tqdm(range(cur_epoch + 1, args.epochs + 1)):
...
def train_epoch(args,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if r < args.mix_p and mixup == 'cut':
            ...
            if not args.patches:
                bbx1, bby1, bbx2, bby2 = rand_bbox_patch(input.size(), lam)
                input[:, :, bbx1:bbx2, bby1:bby2] = input[rand_index, :, bbx1:bbx2, bby1:bby2]
                ratio = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (input.size()[-1] * input.size()[-2]))
            else:
                Imagsize = input.size()
                HalfImagsizeW = Imagsize[2]//2
                HalfImagsizeH = Imagsize[3]//2
                bbx1, bby1, bbx2, bby2 = rand_bbox(Imagsize, lam)
                input[:, :, bbx1:bbx2, bby1:bby2] = input[rand_index, :, bbx1:bbx2, bby1:bby2]
                input[:, :, HalfImagsizeW+bbx1:HalfImagsizeW+bbx2, bby1:bby2] = input[rand_index, :, HalfImagsizeW+bbx1:HalfImagsizeW+bbx2, bby1:bby2]
                input[:, :, bbx1:bbx2, HalfImagsizeH+bby1:HalfImagsizeH+bby2] = input[rand_index, :, bbx1:bbx2, HalfImagsizeH+bby1:HalfImagsizeH+bby2]
                input[:, :, HalfImagsizeW+bbx1:HalfImagsizeW+bbx2, HalfImagsizeH+bby1:HalfImagsizeH+bby2] = input[rand_index, :, HalfImagsizeW+bbx1:HalfImagsizeW+bbx2, HalfImagsizeH+bby1:HalfImagsizeH+bby2]
                ratio = 1 - ((bbx2 - bbx1) * (bby2 - bby1)*4 / (input.size()[-1] * input.size()[-2]))
            output = model(input) # When apply patch, output has [size batch size, N classes, 2,2]

            if args.patches:
                target_b = target_b.repeat_interleave(4)
                target = target.repeat_interleave(4)
                output = output.transpose(1,3).flatten(1,2).flatten(0,1)
